...
(function () {
  ...
  var ui = {
    ...
    indexSlide: 0, fundChartMode: null
  };
  var tickerTimer = null;
  var gFundChart = null; // 当前详情页图表状态（供拖动绑定/切换模式使用）
  ...
  // 取今日有效估值：优先 f.estimate 单点，回退 f.estimateCurve 末点。
  // 关键：保证首页/聚合与详情页图表用同源数据，避免“首页0、详情-2.94%”这类分叉。
  function todayEstimateOf(f) {
    if (f.estimate && f.estimate.date === todayStr() && Number.isFinite(f.estimate.chg)) return f.estimate;
    if (f.estimateCurve && f.estimateCurve.length) {
      var c = f.estimateCurve[f.estimateCurve.length - 1];
      if (c && Number.isFinite(c.nav) && Number.isFinite(c.chg)) {
        return { nav: c.nav, chg: c.chg, date: todayStr(), time: c.time || nowHM(), nav2: c.nav2, chg2: c.chg2 };
      }
    }
    return null;
  }
  ...
  function displayOf(f) {
    ...
    var te = todayEstimateOf(f);
    if (te) {
      return {
        nav: te.nav, chg: te.chg, chgToday: te.chg,
        label: trading ? ('盘中估值 ' + te.time) : '收盘估值(预估) 15:00',
  ...
  function aggregate(f, hs) {
    ...
    (hs || []).forEach(function (h) {
      ...
      if (d.chgToday != null) {
        ...
        today += mv * r;                           // 今日收益 = 当前市值 × 涨跌幅
        todayPrev += mv;                           // 分母用当前市值（MV 加权收益率）
        hasToday = true;
      }
    });
    return {
  ...
  function calcTodayTrend(cards) {
    ...
    return times.map(function (t) {
      var tm = timeToMin(t), totalMV = 0, totalProfit = 0;
      series.forEach(function (pts) {
        var last = null;
        for (var i = 0; i < pts.length; i++) {
          if (timeToMin(pts[i].time) <= tm) last = pts[i]; else break;
        }
        if (last) { totalMV += last.mv; totalProfit += last.profit; }
      });
      return { time: t, profit: totalProfit, pct: totalMV > 0 ? totalProfit / totalMV * 100 : 0, mv: totalMV, prevMv: totalMV - totalProfit };
    });
  }

  // ---------------- 单基金详情页图表 ----------------
  function totalSharesOf(f) {
    var s = 0; holdingsOf(f).forEach(function (h) { if (h && h.shares > 0) s += h.shares; }); return s;
  }
  // 单基金今日分时估值曲线（收益/收益率），无持仓时 profit 记 0（仅收益率有意义）
  function fundTrendData(f) {
    var d = displayOf(f);
    if (d.chgToday == null || !Number.isFinite(d.nav)) return null;
    var curve = (f.estimateCurve || []).filter(function (p) { return p && p.time; });
    if (curve.length < 1) return null;
    var shares = totalSharesOf(f);
    var et = (f.estimate && f.estimate.time) ? f.estimate.time : nowHM();
    var pts = curve.filter(function (p) { return p.time !== et; }).map(function (p) {
      var nv = Number.isFinite(p.nav) ? p.nav : 0;
      var chg = Number.isFinite(p.chg) ? p.chg : 0;
      var mv = nv * (shares > 0 ? shares : 1);
      var profit = shares > 0 ? mv * (chg / 100) : 0;
      return { time: p.time, mv: mv, prevMv: mv - profit, profit: profit, pct: chg };
    });
    var mv = d.nav * (shares > 0 ? shares : 1), chg = d.chgToday, profit = shares > 0 ? mv * (chg / 100) : 0;
    pts.push({ time: et, mv: mv, prevMv: mv - profit, profit: profit, pct: chg });
    return pts;
  }
  // 历史净值走势 → 指数式分时结构（时间按索引均摊到 9:30–15:00）
  function fundHistoryData(f) {
    var hn = f.history || [];
    if (hn.length < 2) return null;
    var minData = hn.map(function (p, i) {
      var frac = hn.length > 1 ? i / (hn.length - 1) : 0;
      var m = 570 + frac * 270, hh = Math.floor(m / 60), mm = Math.round(m - hh * 60);
      var time = (hh < 10 ? '0' : '') + hh + ':' + (mm < 10 ? '0' : '') + mm;
      return { time: time, price: Number.isFinite(p.nav) ? p.nav : 0, vol: 0, amt: 0 };
    });
    return { minData: minData, prevClose: hn[0].nav };
  }
  // 构建详情页图表所需的全部片段（与“总收益查看页面”同款：高/低标注 + 拖动 + 数据框不超屏）
  function renderFundChart(f, mode) {
    var d = displayOf(f);
    var hasHold = totalSharesOf(f) > 0;
    var defMode = mode || (hasHold ? 'profit' : 'rate');
    var sb = stateBadge(d);
    var sb2 = sb ? { t: sb.t, c: sb.c } : { t: '预估', c: 'est' };
    var todayData = fundTrendData(f);
    if (todayData && todayData.length >= 2) {
      var chart = renderTrendChart(todayData, defMode, 360, 190);
      if (chart.meta) {
        var last = todayData[todayData.length - 1];
        var numVal = defMode === 'profit' ? last.profit : last.pct;
        var numCls = numVal >= 0 ? 'up' : 'down';
        var numTxt = defMode === 'profit'
          ? (numVal >= 0 ? '+' : '−') + '¥' + fmt(Math.abs(numVal), 2)
          : (numVal >= 0 ? '+' : '') + fmt(numVal, 2) + '%';
        var subProfitTxt = (last.profit >= 0 ? '+' : '−') + '¥' + fmt(Math.abs(last.profit), 2);
        var subRateTxt = (last.pct >= 0 ? '+' : '') + fmt(last.pct, 2) + '%';
        var label = defMode === 'profit' ? '当前预估收益' : '当前预估收益率';
        var rowHtml = '<span class="cb-item"><b>' + subProfitTxt + '</b>收益</span>' +
          '<span class="cb-item"><b>' + subRateTxt + '</b>收益率</span>' +
          '<span class="state-badge ' + sb2.c + '">' + sb2.t + '</span>';
        var dragHTML = '<div class="cc"><div class="cc-line"></div><div class="cc-tip"></div></div>';
        return {
          type: 'trend', data: todayData, meta: chart.meta, mode: defMode, sb2: sb2,
          numCls: numCls, numTxt: numTxt, label: label, rowHtml: rowHtml,
          note: '今日分时估值（新浪）· 横轴可拖动查看任意时刻', svg: chart.svg, dragHTML: dragHTML,
          lastView: { numCls: numCls, numTxt: numTxt, subProfitTxt: subProfitTxt, subRateTxt: subRateTxt, sb2: sb2 }
        };
      }
    }
    var hist = fundHistoryData(f);
    if (hist && hist.minData.length >= 2) {
      var chart2 = renderIndexChart(hist.minData, hist.prevClose, 360, 190);
      if (chart2.meta) {
        var last2 = hist.minData[hist.minData.length - 1];
        var chg2 = last2.price - hist.prevClose;
        var pct2 = hist.prevClose ? chg2 / hist.prevClose * 100 : 0;
        var cls2 = chg2 >= 0 ? 'up' : 'down';
        var numTxt2 = fmt(last2.price, 4);
        var rowHtml2 = '<span class="cb-item"><b>' + (chg2 >= 0 ? '+' : '−') + fmt(Math.abs(chg2), 4) + '</b>涨跌</span>' +
          '<span class="cb-item"><b>' + (pct2 >= 0 ? '+' : '') + fmt(pct2, 2) + '%</b>涨跌幅</span>';
        var dragHTML2 = '<div class="cc"><div class="cc-line"></div><div class="cc-tip"></div></div>';
        return {
          type: 'index', data: hist.minData, meta: chart2.meta, idxChg: chg2, sb2: sb2,
          numCls: cls2, numTxt: numTxt2, label: '当前净值', rowHtml: rowHtml2,
          note: '历史净值走势（近60个交易日）· 横轴可拖动查看任意时刻', svg: chart2.svg, dragHTML: dragHTML2,
          lastView: { numCls: cls2, numTxt: numTxt2, rowHtml: rowHtml2 }
        };
      }
    }
    return { empty: '暂无可用曲线数据' };
  }
  ...
  function detailInner(code) {
    ...
    var fc = renderFundChart(f, ui.fundChartMode);
    gFundChart = fc;
    var curveLabel = (fc.type === 'index') ? '历史净值走势' : (marketOpen() ? '今日盘中估值曲线（新浪）' : '今日估值曲线（新浪·至15:00）');
    ...
    html += '<div class="detail-chart" id="detailChart">';
    if (fc.empty) {
      html += '<div class="trend-empty" style="height:160px">' + fc.empty + '</div>';
    } else {
      html += '<div class="chart-big ' + fc.numCls + '">' +
        '<div class="chart-big-label">' + fc.label + '</div>' +
        '<div class="chart-big-num">' + fc.numTxt + '</div>' +
        '<div class="chart-big-row">' + fc.rowHtml + '</div>' +
        '</div>';
      if (fc.type === 'trend') {
        html += '<div class="chart-toggle">' +
          '<button class="tog ' + (fc.mode === 'profit' ? 'on' : '') + '" data-act="fModeProfit">收益</button>' +
          '<button class="tog ' + (fc.mode === 'rate' ? 'on' : '') + '" data-act="fModeRate">收益率</button>' +
          '</div>';
      }
      html += '<div class="chart-wrap" id="fWrap">' + fc.svg + fc.dragHTML + '</div>';
      html += '<div class="chart-note">' + fc.note + '</div>';
    }
    html += '</div>';
    html += '<div>';
  ...
  function renderDetail() { view.innerHTML = detailInner(ui.selectedCode); bindFundChartDrag(); }
  ...
  // 横轴拖动：显示任意时刻的盈亏值 / 收益率（container 可指定作用域，默认模态根）
  function bindChartDrag(data, meta, mode, scope, sb2, lastView, container) {
    var root = container || $('#modalRoot');
    var wrap = root.querySelector('.chart-wrap');
    var cc = root.querySelector('.cc');
    var ccLine = root.querySelector('.cc-line');
    var ccTip = root.querySelector('.cc-tip');
    var bigEl = root.querySelector('.chart-big');
    var bigLabel = root.querySelector('.chart-big-label');
    ...
    function showAt(xvb) {
      ...
      ccTip.innerHTML = '<div class="cc-time">' + r.time + '</div>' +
        ...
        '<div class="cc-row"><span>收益率</span><b>' + (r.pct >= 0 ? '+' : '') + fmt(r.pct, 2) + '%</b></div>';
      var nv = r.v;
      bigEl.className = 'chart-big ' + (r.v >= 0 ? 'up' : 'down');
  ...
  function bindIndexDrag(minData, meta, idx, container) {
    var root = container || $('#modalRoot');
    var wrap = root.querySelector('.chart-wrap');
    var cc = root.querySelector('.cc');
    var ccLine = root.querySelector('.cc-line');
  ...
  // 详情页图表拖动绑定（按容器作用域，避免与模态图表冲突）
  function bindFundChartDrag() {
    var wrap = $('#fWrap'); if (!wrap) return;
    var fc = gFundChart; if (!fc || !fc.meta) return;
    var container = wrap.parentElement; // .detail-chart
    if (fc.type === 'trend') {
      bindChartDrag(fc.data, fc.meta, fc.mode, 'fund', fc.sb2, fc.lastView, container);
    } else {
      bindIndexDrag(fc.data, fc.meta, { chg: fc.idxChg || 0 }, container);
    }
  }
  ...
  function onClick(e) {
    ...
    if (el) {
      ...
      switch (act) {
        ...
        case 'fModeProfit': ui.fundChartMode = 'profit'; render(); break;
        case 'fModeRate': ui.fundChartMode = 'rate'; render(); break;
  ...
  function init() {
    ...
    setInterval(function () { refreshAll(); }, 30000); // 基金估值 30s 自动轮询（实时刷新；refreshAll 内置 refreshing 防重入）
  }
